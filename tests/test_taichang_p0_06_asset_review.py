...
class TaichangP006AssetReviewTest(unittest.TestCase):
    ...
    def test_review_package_covers_all_candidates_without_overlap(self):
        ...
        self.assertEqual(metadata["review_candidate_count"], 461)
        self.assertEqual(metadata["review_group_count"], 65)
        self.assertEqual(metadata["auto_blocked_or_reference_count"], 435)
        ...
        self.assertEqual(len(review_ids | blocked_ids), 896)

    def test_default_package_has_no_implicit_approval(self):
        self.assertEqual(self.package["approved_decisions"], [])
        self.assertEqual(self.package["ready_for_ingestion"], [])
    ...
    def test_groups_reduce_page_level_work_without_losing_candidates(self):
        ...
        self.assertEqual(sum(group["candidate_count"] for group in self.package["groups"]), 461)
    ...
    def test_incomplete_approval_is_rejected(self):
        ...
        self.assertEqual(result["approved_decisions"], [])
        self.assertEqual(result["ready_for_ingestion"], [])
    ...
    def test_valid_simulated_knowledge_approval_can_pass_strict_gate(self):
        ...
        self.assertEqual(result["metadata"]["approved_decision_count"], 1)
        self.assertEqual(result["metadata"]["ready_for_ingestion_count"], 1)
        self.assertEqual(result["ready_for_ingestion"][0]["quality_tier_after_review"], "knowledge_only")
    ...
    def test_parameter_candidate_requires_structured_extraction_not_direct_ingestion(self):
        ...
        result = apply_review_decisions(self.package, decisions)
        self.assertEqual(result["metadata"]["validation_error_count"], 0)
        self.assertEqual(result["approved_decisions"][0]["ingestion_action"], "structured_extraction_required")
        self.assertFalse(result["approved_decisions"][0]["ready_for_ingestion"])
        self.assertEqual(result["ready_for_ingestion"], [])
    ...
    def test_workbook_has_required_sheets_validations_and_roundtrip(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(wb.sheetnames, ["审批说明", "证据分组", "候选审批", "自动阻断与仅参考"])
            self.assertEqual(wb["证据分组"].max_row, 66)
            self.assertEqual(wb["候选审批"].max_row, 462)
            self.assertEqual(wb["自动阻断与仅参考"].max_row, 436)
            self.assertGreaterEqual(len(wb["候选审批"].data_validations.dataValidation), 8)
            ws = wb["候选审批"]
