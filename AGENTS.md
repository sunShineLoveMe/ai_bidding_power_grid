...
- A4；
- 上下边距 `2.5cm`，左边距 `2.8cm`，右边距 `2.5cm`；
- 正文仿宋_GB2312 小四 `12pt`；
- 正文 `1.5` 倍行距；
- 正文首行缩进 `2` 字符；
- 一级标题黑体 `22pt` 居中；
- 二级标题黑体 `15pt` 左对齐；
- 三级标题楷体_GB2312 `14pt` 加粗；
- 表格文字仿宋_GB2312 `12pt`，垂直居中；
- 目录标题为 `目  录`；
- 目录页码右对齐，并使用点引导线；
- 页眉左侧为项目名称，右侧为文件类型；
- 页脚居中使用 `第 X 页 共 Y 页` 连续页码和总页数字段；
...
1. 先确认格式来源优先级：招标文件明确格式要求 > 客户可编辑 Word 模板 > `formal_bid_standard` 默认模板 > 高级自定义格式。
2. 默认导出不得让普通用户直接承担全量排版配置风险；MVP 默认使用“国网正式通用标书格式”。
