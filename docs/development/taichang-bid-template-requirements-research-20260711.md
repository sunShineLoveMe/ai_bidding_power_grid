...
2. 对两份泰昌补充 Word 同时做章节/表单结构 inventory、来源属性标注、原始文件回溯、页级去重和证据包完整性盘点。
3. 将 SL265A 当次结构与两份泰昌固定骨架做一次确定性差异对照，形成 V1 对照规则，不建设通用相似度模块。
4. 产出“已有/新增/冲突/缺失/禁止”五类资产清单。
5. 由泰昌投标人员确认章节来源、目录差异和高风险规则。
