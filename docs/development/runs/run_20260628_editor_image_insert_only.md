...
## 2026-06-28 多图插入与父章节状态联动

### 问题

- 客户要求在线编辑器插入图片时支持一次选择多张图片插入正文。
- 目录树中父级大章节只作为结构容器，本身没有正文；旧逻辑把非叶子章节固定视为未完成，导致下级小章节都已完成时父章节状态点仍是灰色。

### 修复

- 资产库图片选择从单选改为多选：
  - 点击图片卡片或“选择”按钮可选中/取消选中。
  - 弹窗显示已选图片数量。
  - 确认按钮按数量显示“插入已选 N 张图片”。
  - 确认后按用户选择顺序批量插入正文，保存章节后继续走现有 DOCX 图片导出链路。
- 本地上传仍保持单张上传并要求图片标题，因为该流程会写入正式资产 metadata。
- 父章节状态改为由下级叶子章节汇总：
  - 所有叶子子章节均完成时，父章节显示绿色完成状态。
  - 任一子章节未完成时，父章节保持未完成。
  - 子章节生成中或失败时，父章节状态同步显示生成中或失败。
...
- 多图插入：
  - 资产库弹窗加载图片 `409` 张。
  - 选择前正文图片数 `0`。
  - 选择 2 张图片后，卡片选中数 `2`，确认按钮显示“插入已选 2 张图片”。
  - 点击确认后正文图片数 `0 -> 2`，`inserted=2`。
- 父章节状态：
  - `1. 投标函及法定格式文件` 状态为 `chapter-status done`。
  - `1.1 投标函及投标函附录`、`1.2 法定代表人身份证明及授权委托书`、`1.3 投标保证金及基本账户资料` 均为 `chapter-status done`。
  - `2. 商务响应文件` 及其已完成子章节均显示 `chapter-status done`。
