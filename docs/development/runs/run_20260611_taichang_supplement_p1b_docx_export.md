# run_20260611_taichang_supplement_p1b_docx_export — 泰昌补充资料 DOCX 真实导出验证

- 生成时间：2026-06-11T21:47:08
- 项目 ID：`4bc3ee73-9ec5-4184-aafd-eaede9f90798`
- 状态：PASS

## 输出文件

- Markdown：`outputs/Guo_Wang_Liao_Zhu_Dian_Li_2025Nian_Di_San_Ci_Wu_Zi_Xie_Yi_Ku_Cun_Zhao_Biao_Cai_Gou/国网辽宁电力2025年第三次物资协议库存招标采购投标文件-河北泰昌电力器材科技有限公司-图文.md`
- DOCX：`outputs/Guo_Wang_Liao_Zhu_Dian_Li_2025Nian_Di_San_Ci_Wu_Zi_Xie_Yi_Ku_Cun_Zhao_Biao_Cai_Gou/国网辽宁电力2025年第三次物资协议库存招标采购投标文件-河北泰昌电力器材科技有限公司-图文.docx`

## 图片选择

| 指标 | 数量 |
| --- | ---: |
| candidates | 597 |
| selected | 24 |
| supplement selected | 18 |
| DOCX media files | 22 |

## 补充资料证据类型

| evidence_type | 数量 |
| --- | ---: |
| `business_license` | 2 |
| `certification` | 5 |
| `inspection_report` | 1 |
| `production_capacity` | 2 |
| `project_performance` | 4 |
| `testing_capacity` | 4 |

## 转换与刷新

- 图片转换：found/inserted/skipped/failed = 24/24/0/0
- 字段刷新：`refreshed`

## 检查结论

- P1B-2 真实标书生成链路引用验证通过。
- WARN：brand logo is in asset library but not yet inserted into DOCX cover/header
