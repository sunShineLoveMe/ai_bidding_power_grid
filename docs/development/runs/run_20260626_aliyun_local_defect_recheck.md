...
- `GET /api/bidding/history` 已改为按当前 `bid_sections` 真实叶子章节统计；
- 首页、历史记录、编辑器均展示同一正文进度口径；
- 真实项目 `a1d853bc-ca4e-43b4-bbea-256f561c8a3d` 历史接口与编辑器等价统计均为 `75/75`、`173534` 字。
- 验证记录：`docs/development/runs/run_20260627_p1_4_progress_and_p2_experience_regression.md`。
...
- 产品库加载中显示 `正在加载产品资料...`，不显示 `暂无产品资料`；
- 资信库加载中显示 `正在加载资信资料...`，不显示 `暂无资信文件`。
...
- `/api/health` 和 `/api/ready` 已返回后端 `commit/branch/builtAt`；
- 前端启动日志同时输出前端 build info 和后端 health version；
- 本地前后端均为 `5b198ff908bc`。
...
- 当前 API 输出统一为北京时间 `+08:00`；
- 真实章节导出任务 `64491e86-9ff2-4762-8466-258f1a3ee315` 验证 `created_at <= started_at <= finished_at`。

### P3：缩略图重复请求与开发模式重复请求

状态：缩略图重复请求已完成本地修复与真实回归；开发模式 StrictMode 重复请求仍不作为生产阻断。
...
- `AuthenticatedImage` 已增加受保护图片 object URL 缓存与引用计数；
- 产品详情同一图片重复打开，第二次新增图片请求数为 `0`。
