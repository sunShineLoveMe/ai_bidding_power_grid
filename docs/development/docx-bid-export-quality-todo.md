...
- 2026-06-29 P1-1 历史父级容器正文治理完成：新增 `scripts/rag/govern_bid_container_content.py`，当前项目 27 个父级容器历史正文已备份到 `docs/development/runs/run_20260629_bid_container_content_governance_container_content_backup.json`，父级 `content` 已清空，metadata 写入 `container_content_policy=ignored_for_formal_export` 和 `migration_review_required=true`；75 个叶子正文保持不变。清理后真实 DOCX 导出字段刷新 `refreshed`，内部提示和治理 metadata 未进入 Markdown/DOCX。记录见 `docs/development/runs/run_20260629_bid_container_content_governance.md`。
