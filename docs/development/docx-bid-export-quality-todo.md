...
- 2026-06-29 P1-1 历史父级容器正文治理完成：新增通用脚本 `scripts/rag/govern_bid_container_content.py`，父级容器数量按项目目录树动态计算。本次辽宁项目实测 27 个父级容器，其历史正文已备份到 `docs/development/runs/run_20260629_bid_container_content_governance_container_content_backup.json`，父级 `content` 已清空，metadata 写入 `container_content_policy=ignored_for_formal_export` 和 `migration_review_required=true`；75 个叶子正文保持不变。清理后真实 DOCX 导出字段刷新 `refreshed`，内部提示和治理 metadata 未进入 Markdown/DOCX。记录见 `docs/development/runs/run_20260629_bid_container_content_governance.md`。
- 2026-06-29 商务标/完整投标文件正文编号错乱修复完成：导出层不再信任正文内旧模板编号、加粗伪标题和 `BID_BODY_SUBHEADING` 完整父级路径，正文伪标题统一压为当前正式章节下一级小标题；完整投标文件同时包含商务/技术分册时启用 `sgcc_mixed` 编号。真实项目 `ae6b7da5-a7b3-473f-a28f-13990ead304e` 技术标、商务标、完整投标文件均完成 `build_project_bid_markdown -> convert_md_to_word -> refresh_docx_fields_with_soffice`，字段刷新 `refreshed`；Markdown/DOCX 审计确认 `2.10.1.1.2 投标函`、深层商务表单串号、泛化“商务分册/商务文件”小标题均为 `0`。记录见 `docs/development/runs/run_20260629_docx_numbering_fix_regression.md`。
...
- 背景：客户资料和实际投标交付通常按技术标、商务标分别下载；原页面只有单一“标书下载”入口，导出完成后依赖自动打开下载地址，浏览器弹窗/不安全下载策略可能造成客户以为导出失败。
- 页面调整：标书编制页顶部“正式检查”旁改为“导出投标文件”下拉按钮，提供“下载技术标 DOCX”“下载商务标 DOCX”“下载完整投标文件 DOCX”三个入口；完成后弹窗展示交付包、文件名、文件大小、字段刷新、图片插入统计，并提供明确主按钮“下载技术标 DOCX / 下载商务标 DOCX / 下载完整投标文件 DOCX”。
- 构建检查：`cd frontend && npm run build` 通过；保留既有 Vite 动静态混用导入和 chunk size 警告。`python3 -m py_compile backend/api/assets.py backend/api/knowledge.py backend/db/supabase_repo.py` 通过。
- 真实浏览器回归：Chrome CDP 打开 `http://127.0.0.1:5173/bid-editor?projectId=a1d853bc-ca4e-43b4-bbea-256f561c8a3d`，确认下拉菜单三项均可见。
- 技术标真实任务：`8caee0be-9b2e-4ab6-9b43-e0bae74b78b1`，请求体 `volumeType=technical`，任务完成后弹窗显示“技术标 DOCX 已生成”，字段刷新已完成，图片插入 `24` 张、跳过 `0`、失败 `0`，文件大小 `4.84 MB`；点击“下载技术标 DOCX”触发真实浏览器 download 事件并保存成功。
- 商务标真实任务：`deed0129-d229-4bae-8aeb-b8e04308fd19`，请求体 `volumeType=business`，任务完成后弹窗显示“商务标 DOCX 已生成”，字段刷新已完成，图片插入 `18` 张、跳过 `0`、失败 `0`，文件大小 `4.17 MB`；点击“下载商务标 DOCX”触发真实浏览器 download 事件并保存成功。
- 截图与下载样本：`output/playwright/local-p1-3-export-completion-20260627/`。
- 记录：`docs/development/runs/run_20260627_p1_3_volume_export_download_completion.md`。
