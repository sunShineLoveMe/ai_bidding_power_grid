...
| 进行中（本地收口，待云上发布复测） | 阿里云企业库展示与来源收敛 | 企业知识库 / 云环境 | 客户正在真实试用；来源混入、分类不准、入口不稳定会直接影响可信度 | 本地已完成证书来源严格收敛、CPVC 报告规格覆盖约束和 80 入口配置文档；Base+泰昌专项门禁、资质证书 stream、CPVC stream 均 PASS。云上当前 `:8080` 健康，80 仍 `Empty reply from server`，发布并复测后关闭 | `docs/rag/todo.md` P1C-15、`docs/rag/runs/run_20260626_aliyun_enterprise_source_convergence_final_review.md`、`docs/rag/runs/run_20260624_aliyun_online_rag_regression.md` |
