...
| 进行中（本地热修通过，待云上发布复测） | 阿里云企业库展示与来源收敛 | 企业知识库 / 云环境 | 客户正在真实试用；来源混入、分类不准、入口不稳定会直接影响可信度 | 阿里云 `f97b31a` 复查发现资质证书过滤顺序仍可能让误标 `certification` 的 ESG/绿色发展/废水资料绕过；本地已追加负向来源优先、去掉泛化“认证证书”放行、并过滤资质证书查询下的图片资产输入。`run_20260626_aliyun_cert_negative_filter_hotfix` 门禁 PASS，资质证书 stream 仅三体系认证来源；提交并发布新 commit 后复测线上 API/页面再关闭 | `docs/rag/todo.md` P1C-15、`docs/rag/runs/run_20260626_aliyun_cert_negative_filter_hotfix_summary.md`、`docs/rag/runs/run_20260626_aliyun_enterprise_source_convergence_final_review.md`、`docs/rag/runs/run_20260624_aliyun_online_rag_regression.md` |
