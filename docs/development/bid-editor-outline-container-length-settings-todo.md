...
| P1-1 | 已完成 | 清理/隔离历史父级内容 | 数据治理脚本/一次性修复 | 已新增可 dry-run/apply 的治理脚本；当前项目 27 个父级容器历史正文已备份、清空并写入 `container_content_policy=ignored_for_formal_export`；全部标记为 `migration_review_required=true`，后续如需复用只能人工迁移到叶子小节。 |
