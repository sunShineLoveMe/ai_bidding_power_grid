...
| [x] | P2 正文内容复用、正式证据编排与用户入口收口 | `taichang_bid_evidence_mapping.json`、P1-05 结构化 RAG 基线、`chapter_content_manifest`、`backend/services/taichang_evidence_export.py`、`backend/services/formal_export_delivery_gate.py`、`frontend/src/components/home/TaichangReuseCard.tsx`、P2-04～P2-06 运行记录 | P2-02～P2-06 已完成当前 SL2655 的正文、原表、正式证据编排、二阶段交付门禁和首页入口：真实整本 DOCX 选中/插入/失败 `6/6/0`，原页序、正式资产、四类源表、字段刷新和 DOCX 可见内容审计均 PASS；133 条 `knowledge_only` 图片未进入 DOCX。当前 9 个业务阻断和 1 个成品就绪阻断继续使文件保持草稿版；首页泰昌单一入口显式使用 `taichang_reuse`，通用模式保持 `general` |
...
### 2026-07-14 泰昌 P1-05 结构化事实 RAG 基线收口

- 已建立 27 个父块、139 个子块，共 166 块；参数、业务台账、证据包和业绩记录均保留结构化行/ID 直查，不只依赖普通向量文本。
- 139 个子块 embedding 完整，五类范围 metadata 完整；第二次执行复用同一数据库文档并替换原 166 块，未重复累加。
- 知识库召回前按企业、来源域、产品族、物料类别和证据类型过滤；写作场景可从子块回溯报告或证据包父块。
- 新增架空绝缘导线跨产品负样本和范围守卫；修复“参数”泛词误召回 MPP 结构化事实，真实 stream 不再串入其他产品。
- Base 30 + 泰昌专项 30 双模式门禁 PASS，qwen3 下禁用关键词和跨角色串扰均为 0；真实 stream 3/3、后端全量 438 passed 及 2 个子测试通过。
- 本轮资产质量等级提升 0、DOCX 选图变更 0，不触发正式 DOCX XML 审计。
