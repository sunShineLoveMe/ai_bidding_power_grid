...
## Run 61 — 泰昌历史标书复用 P1-03 结构化参数和业务台账（2026-07-14）

> 总记录：`docs/rag/runs/run_20260714_taichang_p1_03_business_ledgers_summary.md`
> 历史策略说明：本 Run 的人员受限策略已由 Run 63 按客户确认调整为私有项目完整人员数据可用。

### 触发原因

P1-01 已完成低风险知识资产入库，P1-02 已建立 16 个文件级证据包。P1-03 需要在不重新解析、不重复入库的前提下，把产品参数、设备与校准证书、管理体系证书、审计、人员、检验报告登记和项目业绩收敛为可追溯业务台账，并接入真实知识库问答。

### 数据结果

| 验证项 | 结果 |
| --- | --- |
| 业务台账 | PASS，84 行；业务主键唯一；版本库只保存 17 行安全投影 |
| 既有结构化层复用 | PASS，产品参数 36、历史参数候选 20、项目业绩证据 2、证据包 16 |
| 来源页码 | PASS，产品参数重抽取补齐第 3/4 页；设备、证书、审计、人员均保留来源页 |
| 时效门禁 | PASS，职业健康安全证书已过期阻断；设备采用 2026 原始校准证书 |
| 补证边界 | PASS，N-HAP/UPVC 缺原始报告不生成参数；2024 审计编号不推断；知识产权无原件不生成事实 |
| 人员权限 | PASS，67 条个人明细均 `restricted/internal_only` 且仅写本地 staging，不进入 Git；普通问答只展示脱敏汇总 |
| 数据库/资产/DOCX | 写入 0、提升 0、选图变更 0 |

### 真实链路与回归

首轮真实 stream 暴露“多年度台账被来源去重合并”和“N-HAP/UPVC 问答串入 CPVC”问题；已改为按业务对象分组，并按明确产品族过滤结构化上下文、向量结果和图片资产。第二轮 7/7 PASS，覆盖设备、过期证书、缺原件报告、三年审计、人员脱敏、MPP 环刚度、CPVC 平均内径/壁厚。
...
P1-03 完成。所有精确值均可追溯，缺证事项不升级为企业事实，人员受限字段未进入普通问答。本轮不改变正式图片资产或 DOCX 导出，因此不执行 DOCX XML 审计。下一步为 P1-04 章节—事实—证据映射。

---

## Run 62 — 泰昌历史标书复用 P1-04 章节—事实—证据映射（2026-07-14）

> 总记录：`docs/rag/runs/run_20260714_taichang_p1_04_chapter_evidence_mapping_summary.md`
> 历史策略说明：本 Run 的 19 个映射、人员明细纳入 0 已由 Run 63 更新为 20 个映射并纳入 67 行人员明细。

### 触发原因

P1-01 已形成 `knowledge_only` 历史知识资产，P1-02 已形成 16 个文件级证据包，P1-03 已形成可追溯业务台账。P1-04 需要把这些既有资产映射到技术标和商务标语义章节，同时避免固化历史章节号、随机选单页、复制资产或把缺原件事项升级为企业事实。

### 映射结果

| 验证项 | 结果 |
| --- | --- |
| 映射规模 | PASS，技术标 12、商务标 7，共 19 个语义映射 |
| 状态分层 | 已有资料 5、部分可用 10、缺原件 3、人工确认 1 |
| 动态骨架 | PASS，使用语义章节键和标题别名，固定历史章节号 0 |
| 事实和证据 | 关联 15 个唯一证据包、36 行原始参数、17 行安全台账投影和 P1-01 知识资产 |
| 资产复用 | PASS，同一 ID 可跨章节引用，资产复制 0 |
| 产品边界 | CPVC/MPP 可定位；N-HAP/UPVC 缺原件，不生成正式参数或覆盖结论 |
| 时效与权限 | 过期职业健康安全证书引用 0；人员受限明细纳入 0；签章只保留人工确认占位 |
| 跨域边界 | 辽宁仅作当前招标要求；河北豪乾仅作结构和写法参考 |

### 回归

- P1-04 + P1-02 + P1-03 专项：29 passed，1 个既有 PyPDF2 弃用告警。
- 后端全量：432 passed、2 subtests passed、11 个既有弃用告警。
- 本轮数据库写入 0、metadata 改写 0、资产复制/提升 0、召回策略变更 0、接口变更 0、DOCX 选图变更 0。

### 结论

P1-04 完成。每个标记为“已有资料”的章节均能定位事实或证据包；缺原件、过期证书、评分项选择和客户承诺均未越权自动生成。本轮只是离线语义映射产物，不改变 RAG 语料、metadata、召回、真实问答接口或 DOCX 导出，因此不执行 Base+泰昌召回门禁、真实 stream 或 DOCX XML 审计。下一步进入 P1-05 RAG 入库与回归门禁。

---

## Run 63 — 泰昌私有项目完整人员数据策略调整（2026-07-14）

> 总记录：`docs/rag/runs/run_20260714_taichang_private_personnel_policy_adjustment_summary.md`

### 触发原因

客户确认该项目为私有项目，不需要因人员姓名、岗位、证书编号等信息设置隐私阻断。原 P1-03 将 67 行人员明细留在本地受限 staging、原 P1-04 不映射人员章节，会损失标书编制所需的企业资信事实，因此调整为泰昌租户内完整可用。

### 数据与映射结果

| 验证项 | 结果 |
| --- | --- |
| P1-03 完整台账 | PASS，84/84 行版本化并可供私有知识库查询 |
| 人员花名册 | PASS，65 行，保留姓名、岗位、学历、社保/劳动合同记录、入职年份和来源页 |
| 人员证书 | PASS，2 行，保留完整证书编号、准操项目、初领/复审/有效期和来源 |
| 隐私阻断 | 0 行；策略依据写入 manifest 和 P1-04 mapping |
| P1-04 映射 | 技术 12、商务 8，共 20；新增人员组织与人员证书章节 |
| 保留门禁 | 证书有效期、项目岗位适配、项目任命、授权签章、来源真实性继续复核 |
| 数据库/资产/DOCX | 写入 0、等级提升 0、选图变更 0 |

### 真实链路与回归

- 真实 `/api/knowledge/search/stream`：7/7 PASS，人员用例返回姓名、岗位、完整证书编号、准操项目、有效期和来源。
...
策略调整完成。人员完整数据现在是泰昌私有项目可引用企业事实，不再因隐私策略被裁剪；真实性、时效性、跨资料域隔离及授权签章确认仍保持。标准增量门禁 PASS，本轮不涉及 DOCX 选图或导出逻辑，无需执行 DOCX XML 审计。下一步进入 P1-05 生成链路接入。

---

## Run 64 — 泰昌历史标书复用 P1-05 结构化事实 RAG 入库与回归（2026-07-14）

> 总记录：`docs/rag/runs/run_20260714_taichang_p1_05_structured_rag_summary.md`

### 入库与检索结果

| 验证项 | 结果 |
| --- | --- |
| 父子分块 | PASS，27 parent + 139 child，共 166 块 |
| Embedding | PASS，139/139 子块完整 |
| 范围 metadata | PASS，`enterprise/source_domain/product_family/material_category/evidence_type` 全部完整 |
| 结构化来源 | 参数 36、完整业务台账 84、证据包 16、业绩证据 2 |
| 幂等刷新 | PASS，同一数据库文档，166 块替换 166 块，数据集哈希不变 |
| 子召回父回溯 | PASS，MPP 环刚度可回溯报告父块和证据包父块 |
| 跨产品范围 | PASS，架空绝缘导线仅命中范围守卫，其他产品事实 0 |

### 真实链路与回归

- 首轮真实 stream 发现结构化参数直查因“参数”泛词误召回 MPP；增加产品范围硬门禁后 3/3 PASS。
- MPP 环刚度、CPVC 平均内径/壁厚均返回准确数值、报告编号和来源页。
...
P1-05 完成。泰昌结构化事实同时具备精确行直查、metadata 预过滤、子块语义召回和父块写作回溯；跨产品范围守卫已纳入标准 30 条泰昌困难样本。普通投标项目不触发基线重建。本轮资产等级提升和 DOCX 选图变更均为 0。

---

## Run 65 — P2-01 项目规则与动态骨架（2026-07-14）

> 总记录：`docs/development/runs/run_20260714_taichang_p2_01_project_skeleton.md`

### 真实链路与回归

- SL2655 原生 DOCX 格式表生成 43 条规则，纳入 19、排除 24；价格按包、商务/技术按分标，无跨分册串用。
- 真实目录 SSE 返回动态骨架并持久化；1440×960 浏览器目录页显示“目录已按本次招标文件核定”，控制台错误 0。
- 真实 `/api/knowledge/search/stream` 正确回答泰昌 MPP `66.40 kN/m²` 仅对应 `250×22` 报告，不能证明 SL2655 `200×14` 规格。
- 后端全量：446 passed、2 subtests passed；前端生产构建 PASS。
...
P2-01 完成，门禁 PASS。当次招标文件决定目录和条件状态，历史标书只作确定性差异对照；规格缺口不会被“同产品族”掩盖。下一任务为 P2-02 泰昌专版分册骨架与正文内容复用。

---

## Run 66 — P2-02 真实项目章节正文复用（2026-07-14）

> 总记录：`docs/development/runs/run_20260714_taichang_p2_02_real_project_integration.md`

### 真实项目与生成链路

- 真实项目 `890068b5-41fa-4f7a-bf4e-fd0169c489ec` 使用 SL2655 原生 DOCX，未调用 MinerU/OCR；解析得到 80 条要求、60 条风险、70 条评分项、8 个项目分块和 19 个动态章节。
- 首次“投标人基本情况表”真实模型生成约 3,440 字，但补写未证实许可、认证、财务/诉讼和承诺事实，判定为未通过；修复后精确工商事实与 CPVC/MPP 参数走生产级确定性渲染，不让模型重写结构化精确值。
- “投标人基本情况表”和“技术特性参数表”均通过真实章节 SSE 写入数据库并在 1440×960 浏览器读取；前者只含 7 个核验字段，后者使用 17 行结构化参数、19 个定向分块和检验报告证据包，控制台错误 0。
- 真实 `/api/knowledge/search/stream` 首次遇到外部模型 503，未记为通过；恢复后重试 HTTP 200、召回 3 条资料、错误事件 0，正确给出报告号 `2024100312005501712`、`250×22`、`176%`，并明确不能覆盖 SL2655 `200×14 / ≥200%`。
- 定向测试 75 passed；后端全量 458 passed、2 subtests passed；前端生产构建 PASS。
...
P2-02 真实项目保存链路和标准增量门禁 PASS。企业事实已实际写入标书章节，不再停留在“知识库可召回”；当前 SL2655 目录的其余 17 个章节属于招标原表/固定表单，按边界进入 P2-03，不允许模型随机扩写。

---

## Run 67 — P2-03 固定表单与参数响应（2026-07-14）

> 总记录：`docs/development/runs/run_20260714_taichang_p2_03_fixed_form_completion.md`
> 增量门禁：`docs/rag/runs/run_20260714_taichang_p2_03_fixed_forms_summary.md`

### 真实链路与回归

- 真实项目 `890068b5-41fa-4f7a-bf4e-fd0169c489ec` 的商务偏差表、人员关系说明、技术偏差表和技术特性参数表均接入当次招标原生 OOXML 表单，模型调用 0。
- 专项技术规范 22 行判定为 `partial_match=10 / mismatch=3 / unknown=9 / exact_match=0`，投标人保证值保持 0。
- 四份 DOCX 真实导出及 LibreOffice 字段刷新完成；技术参数成品保持两张原表、22 行，刷新后合并结构、列宽、源文字和保证值审计 PASS。
- 真实 `/api/knowledge/search/stream` 明确回答 `250×22` 报告不能覆盖 `200×14`；环刚度 `66.40` 和伸长率 `176%` 仅属于报告原规格，不能直接填写。
- 后端全量：465 passed、2 subtests passed；前端生产构建 PASS。

| 测试集 | 模式 | Recall@5 | Top1 | MRR | 禁用关键词 | 跨角色串扰 | 平均耗时 |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |
| Base | off | 96.7% | 100% | 0.944 | 0% | 0% | 289 ms |
| Base | qwen3-rerank | 96.7% | 100% | 0.944 | 0% | 0% | 552 ms |
| 泰昌专项 | off | 93.3% | 100% | 0.917 | 3.3% | 0% | 336 ms |
| 泰昌专项 | qwen3-rerank | 100% | 100% | 0.973 | 0% | 0% | 618 ms |

### 结论

P2-03 完成，门禁 PASS。固定表单不再由 Markdown 或模型重建；未确认的偏差、人员关系和投标人保证值继续阻断正式导出。下一任务进入 P2-04 证据包编排与导出。

---

## Run 68 — P2-04 正式证据包编排与整本 DOCX（2026-07-14）

> 总记录：`docs/development/runs/run_20260714_taichang_p2_04_evidence_bundle_export.md`
> 增量门禁：`docs/rag/runs/run_20260714_taichang_p2_04_evidence_bundle_export_summary.md`

### 真实链路与回归

- 当前 SL2655 按章节 manifest 只选中泰昌营业执照 1 页和 MPP 检验报告 5 页；真实浏览器完整 DOCX 任务 `6099a104-a67f-4337-b6a6-e33e2301ccad` 完成，选中/插入/失败 `6/6/0`，字段刷新 `refreshed`。
- 证据包按原页序整包输出；CPVC、项目业绩、审计、体系证书、设备、133 条 `knowledge_only` 图片、辽宁和河北豪乾资料均未进入 DOCX。
- 整本 DOCX 的四类固定表单均替换为当次招标源表 OOXML；LibreOffice 刷新后原文、合并关系和列宽审计 PASS，技术参数保证值 0。
- DOCX 媒体对象 6 个，用户可见禁用题注、内部 API/解析路径和跨资料域来源命中 0。
- 后端全量：471 passed、2 subtests passed；前端生产构建 PASS。

| 测试集 | 模式 | Recall@5 | Top1 | MRR | 禁用关键词 | 跨角色串扰 | 平均耗时 |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |
| Base | off | 96.7% | 100% | 0.944 | 0% | 0% | 251 ms |
| Base | qwen3-rerank | 96.7% | 100% | 0.944 | 0% | 0% | 554 ms |
| 泰昌专项 | off | 93.3% | 100% | 0.917 | 3.3% | 0% | 315 ms |
| 泰昌专项 | qwen3-rerank | 100% | 100% | 0.973 | 0% | 0% | 649 ms |

### 结论

P2-04 完成，门禁 PASS。当前目录没有确定映射的证据不会因资产库存在而被强行插入，完整报告也不会随机抽页。正式检查仍保留 9 个真实业务/保证值阻断，当前输出只标记为草稿版。下一任务进入 P2-05 导出前正式门禁。

---

## Run 69 — P2-05 正式导出与成品交付门禁（2026-07-14）

> 总记录：`docs/development/runs/run_20260714_taichang_p2_05_formal_delivery_gate.md`
> 增量门禁：`docs/rag/runs/run_20260714_taichang_p2_05_formal_delivery_gate_summary.md`

### 真实链路与回归

- 真实浏览器完整 DOCX 任务 `0107ec80-5b55-4493-9b24-e71af0c5f36e` 完成；导出前保留 9 个业务阻断，导出后二阶段成品门禁执行 7 项。
- POST-001 因 26 个正式必填字段未齐阻断；证据包 `6/6/6`、正式资产来源、图片 `6/6/0`、字段刷新、四类源表和 DOCX XML 内部痕迹 6 项 PASS。
- 最终有效门禁 `export_mode=draft / can_formal_export=false`，浏览器弹窗显示“成品门禁 1 项未通过”。
- 后端全量：475 passed、2 subtests passed；前端生产构建 PASS。

| 测试集 | 模式 | Recall@5 | Top1 | MRR | 禁用关键词 | 跨角色串扰 | 平均耗时 |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |
| Base | off | 96.7% | 100% | 0.944 | 0% | 0% | 268 ms |
| Base | qwen3-rerank | 96.7% | 100% | 0.944 | 0% | 0% | 528 ms |
| 泰昌专项 | off | 93.3% | 100% | 0.917 | 3.3% | 0% | 316 ms |
| 泰昌专项 | qwen3-rerank | 100% | 100% | 0.973 | 0% | 0% | 621 ms |

### 结论

P2-05 完成，门禁 PASS。DOCX 成功生成不再等于可正式交付；导出前业务条件和导出后成品质量必须同时通过。当前 SL2655 保持草稿状态，未绕过客户确认和规格证据缺口。下一任务进入 P2-06 用户入口与流程收口。
