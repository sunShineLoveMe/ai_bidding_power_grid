...
- 新增“人员组织与人员证书”章节，可定位完整花名册和人员证书；正式使用仍按项目岗位、证书有效期和实际分工复核。
...
- 专项：30 passed，1 个既有 PyPDF2 弃用告警。
- 后端全量：433 passed、2 subtests passed、11 个既有弃用告警。
- “已有资料”章节均能定位至少一个事实、证据包或知识资产 ID。
- 固定历史章节号、过期职业健康安全证书和未知证据包引用均由自动测试阻断；完整人员台账按客户确认进入私有项目映射。

## 门禁结论

人员结构化召回已调整并执行真实 `/api/knowledge/search/stream` 与 Base+泰昌标准增量门禁，均 PASS。本轮未写数据库、未提升资产等级、未改变 DOCX 选图/导出逻辑，因此不执行 DOCX XML 审计。P1-05 继续完成生成链路接入。
