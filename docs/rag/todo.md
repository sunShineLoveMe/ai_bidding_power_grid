...
| P1C-15 | [~] | 阿里云企业库展示与来源收敛收口 | `backend/api/knowledge.py`、`backend/rag/display_names.py`、`scripts/rag/repair_taichang_green_asset_metadata.py`、`docs/rag/runs/run_20260626_taichang_enterprise_source_scope_final_review.md`、`docs/rag/runs/run_20260626_aliyun_enterprise_source_convergence_final_review.md`、`docs/rag/runs/run_20260624_aliyun_online_rag_regression.md` | 已举一反三完成企业资料来源误解读专项收敛：资质证书、企业证明材料、绿色低碳、人员社保、检验报告、生产制造能力 6 问真实 API 审计 PASS；本地修复 221 条绿色/低碳资产和 1205 条相关 chunk metadata，绿色/ESG/碳足迹/废水资料中 `certification_with_green_terms=0`；标准门禁 `run_20260626_taichang_enterprise_source_scope_final` PASS。需提交发布新 commit 到阿里云，执行同一 metadata 修复脚本并复测线上 API/页面后关闭 |
...
19. **P1C-15 进行中：阿里云企业库展示与来源收敛收口。** 已完成一次性专项收敛：查询侧按证据类型桶收敛资质证书、绿色低碳、人员社保、检验报告、生产制造能力等来源；展示层不再把具体分类覆盖为“泰昌企业资料”；本地执行绿色低碳 metadata 修复后，221 条绿色/低碳资产和 1205 条 chunk 均归入 `green_low_carbon`。6 问真实 API 审计和标准门禁 `run_20260626_taichang_enterprise_source_scope_final` 均 PASS。剩余动作是提交新 commit、发布到阿里云 ECS，执行 `scripts/rag/repair_taichang_green_asset_metadata.py --execute`，并复测 80 入口、线上 API 和真实页面后关闭 P0。
