...
0. **P2-05 导出前正式门禁与导出后成品门禁已完成，下一任务为 P2-06 用户入口与流程收口。** 真实 SL2655 任务 `0107ec80-5b55-4493-9b24-e71af0c5f36e` 保留 9 个导出前业务阻断，并执行 7 项导出后检查：正式必填字段未齐 1 项阻断；证据包页数/manifest/选中 `6/6/6`、正式资产 policy、图片 `6/6/0`、字段刷新、四类固定表单及 DOCX 内部痕迹审计 6 项 PASS。最终任务 `export_mode=draft`，浏览器显示“成品门禁 1 项未通过”，不会因 Word 成功生成而误标正式版。通用项目兼容边界已保留；后端全量 475 passed 及 2 个子测试、前端构建和 Base 30 + 泰昌专项 30 双模式门禁均 PASS。
