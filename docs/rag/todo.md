...
| P1C-15 | [~] | 阿里云企业库展示与来源收敛收口 | `backend/api/knowledge.py`、`backend/rag/retrieval.py`、`docker-compose.yml`、`docs/rag/runs/run_20260626_aliyun_enterprise_source_convergence_final_review.md`、`docs/rag/runs/run_20260626_aliyun_cert_negative_filter_hotfix_summary.md`、`docs/rag/runs/run_20260624_aliyun_online_rag_regression.md` | 阿里云发布 `f97b31a` 后发现资质证书过滤顺序仍可能让误标 `certification` 的 ESG/绿色发展/废水资料绕过；已本地热修为负向词优先、去掉泛化“认证证书”放行、并过滤资质证书查询下的图片资产输入。`run_20260626_aliyun_cert_negative_filter_hotfix` 本地门禁 PASS，真实 stream `raw_contexts` 仅三体系认证证书。需提交发布新 commit 到阿里云后复测线上 API 和页面再关闭 |
...
19. **P1C-15 进行中：阿里云企业库展示与来源收敛收口。** 本地已修复资质证书来源混入 ESG/废水报告、CPVC 报告规格覆盖措辞冲突，并完成 `FRONTEND_HTTP_PORT=80` 入口配置文档；阿里云 `f97b31a` 复查发现“认证证书”泛化词和误标 `certification` 仍可能绕过过滤，已追加 `run_20260626_aliyun_cert_negative_filter_hotfix` 热修门禁，资质证书 stream 仅返回三体系认证来源。剩余动作是提交新 commit、发布到阿里云 ECS，并复测 80 入口、线上 API 和真实页面后关闭 P0。
