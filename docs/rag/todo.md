...
| P1C-15 | [x] | 阿里云企业库展示与来源收敛收口 | `backend/api/knowledge.py`、`backend/rag/display_names.py`、`frontend/src/pages/KnowledgeBase/KnowledgeSearchDrawer.tsx`、`scripts/rag/repair_taichang_green_asset_metadata.py`、`docs/rag/runs/run_20260626_taichang_enterprise_source_scope_final_review.md`、`docs/rag/runs/run_20260626_aliyun_online_source_scope_regression.md`、`docs/rag/runs/run_20260624_aliyun_online_rag_regression.md` | 已举一反三完成企业资料来源误解读专项收敛：资质证书、企业证明材料、绿色低碳、人员社保、检验报告、生产制造能力 6 问真实 API 审计 PASS；本地修复 221 条绿色/低碳资产和 1205 条相关 chunk metadata，绿色/ESG/碳足迹/废水资料中 `certification_with_green_terms=0`；标准门禁 `run_20260626_taichang_enterprise_source_scope_final` PASS。阿里云线上 `ac01c14` 后端/API 核心回归 PASS，`cfd1e4e03dd5` 前端标题补丁发布后 Chrome 页面复验 PASS；资质证书问答未再混入绿色低碳来源，绿色低碳参考来源标题已显示“绿色发展规划报告”“碳足迹报告” |
...
0. **P2-06 主页泰昌历史标书复用入口与用户流程收口已完成，P2-01～P2-06 已闭环。** 首页在现有智能标书和分册工作区之间新增“泰昌历史标书复用”工作卡，只保留“创建泰昌专版标书”单一主操作；原“继续历史标书”已改为“继续已有任务”。普通入口仍提交 `general`，泰昌入口实测提交 `taichang_reuse` 且工作流显示“泰昌专版”，不新增历史模板选择或资产清单步骤。真实浏览器 `1600×1000`、`1280×900` 无文字/按钮错位和横向溢出；前端生产构建、后端全量 475 passed 及 2 个子测试、服务 ready 检查均 PASS。本轮未改变 RAG 语料、metadata、召回、正式资产或 DOCX 规则，不重复执行增量入库和召回门禁；P2-05 已通过的 Base 30 + 泰昌专项 30 结果继续作为当前基线。
0. **P2-04 正式证据包编排与导出已完成，下一任务为 P2-05 导出前正式门禁。** 当前 SL2655 只根据章节内容 manifest 选择泰昌营业执照 1 页和 MPP 检验报告 5 页；真实浏览器完整图文 DOCX 任务 `6099a104-a67f-4337-b6a6-e33e2301ccad` 选中/插入/失败 `6/6/0`、字段刷新成功。证据包执行缺页/顺序/产品/正式资产/预算整包门禁，CPVC、项目业绩、审计、体系证书、设备、133 条 `knowledge_only` 图片及辽宁/河北豪乾资料均未串入。整本 DOCX 四类招标源表刷新后审计 PASS，包内媒体 6 个、内部路径和禁用题注命中 0。后端全量 471 passed 及 2 个子测试、前端构建、Base 30 + 泰昌专项 30 双模式门禁均 PASS；9 个真实业务/保证值阻断继续将当前输出标记为草稿版。
0. **P2-03 固定表单与参数响应已完成，下一任务为 P2-04 证据包编排与导出。** SL2655 当次招标 4 个固定表单已按原生 DOCX OOXML 接入真实项目，22 行技术参数形成四态覆盖 manifest；泰昌现有 `250×22 / 66.40 / 176%` 不得写入项目 `200×14 / ≥200%` 保证值，当前已填 0。四章节真实 SSE 均绕过模型；四份真实 DOCX 经 Celery 和 LibreOffice 刷新，技术参数成品保持两张原表、22 行、合并结构和列宽，刷新后审计 PASS。正式检查 `T-013` 正确阻断 4 个未确认表单。真实知识问答、Base 30 + 泰昌专项 30 门禁、后端全量 465 passed 及 2 个子测试、前端构建均 PASS。
0. **P2-02 泰昌专版分册骨架与正文内容复用已完成真实项目验收，下一任务为 P2-03。** SL2655 真实项目通过原生 DOCX 解析形成 43 条项目规则、19 个动态章节、80 条要求、60 条风险和 70 条评分项；“投标人基本情况表”“技术特性参数表”已通过真实章节 SSE 写入 PostgreSQL 并由浏览器读取。首次真实模型结果因补写未证实许可、认证、财务和承诺内容判定失败；修复后精确工商事实和 CPVC/MPP 参数使用生产级确定性渲染，基本情况只保留 7 个核验字段，MPP 正文使用 17 行结构化参数、19 个定向分块和检验报告证据包，内部 ID、辽宁、河北豪乾、CPVC 跨产品命中均为 0。真实知识问答召回 3 条资料并正确判定 `250×22 / 176%` 不能覆盖 SL2655 `200×14 / ≥200%`；标准增量门禁、定向 75 项、后端全量 458 passed 及 2 个子测试、前端构建均 PASS。其余 17 个招标原表/固定表单不随机扩写，进入 P2-03 原表填充和逐项覆盖判断。
0. **泰昌历史标书复用 P1-05 RAG 入库与回归门禁已完成。** 已将 36 行原始参数、84 行完整业务台账、16 个证据包和 2 行业绩证据组成 27 个父块 + 139 个子块的结构化事实基线，139 个 embedding 完整；第二次幂等执行复用同一数据库文档并以 166 块替换 166 块。召回前按 `enterprise/source_domain/product_family/material_category/evidence_type` 过滤，结构化直查继续作为精确值优先层；新增架空绝缘导线范围守卫并修复泛词误召回 MPP。真实 stream 3/3、标准 Base 30 + 泰昌专项 30 双模式门禁、后端全量 438 passed 及 2 个子测试均 PASS。下一步进入 P2-01 项目级招标编制规则、动态骨架与历史参考骨架差异对照。
0. **泰昌历史标书复用 P1-04 章节—事实—证据映射已完成并按私有项目策略更新。** 已建立 20 个动态语义章节映射（技术标 12、商务标 8），新增人员组织与人员证书章节；状态为已有资料 6、部分可用 10、缺原件 3、人工确认 1。只读复用 15 个唯一证据包、36 行原始报告参数、84 行完整业务台账和 P1-01 知识资产，不复制业务资产。人员明细 67 行已纳入映射；固定历史章节号纳入 0。专项 30 passed、后端全量 433 passed 及 2 个子测试通过；真实人员 stream 和 Base+泰昌标准门禁 PASS。
0. **泰昌历史标书复用 P1-03 完整业务台账已按客户确认策略更新。** 84 行统一台账全部版本化并可在泰昌私有知识库查询，其中人员花名册 65 行、人员证书 2 行、人员摘要 1 行；可返回姓名、岗位、学历、社保/劳动合同记录、完整证书编号、准操项目、有效期和来源页。N-HAP/UPVC 缺原件、2024 审计编号待复核、知识产权无可信原件及过期职业健康安全证书等真实性门禁继续保留。真实 stream 7/7、Base+泰昌标准门禁、后端全量 433 passed 及 2 个子测试均 PASS；数据库写入 0、资产提升 0、DOCX 选图变更 0。
0. **泰昌历史标书复用 P1-01 已完成实际提取、严格去重、知识资产入库和技术参数精度回归。** 169 个自动接收媒体全部从原始 Word 实际提取并核验来源哈希；以真实 PostgreSQL 599 个既有资产重新执行证据身份、字节、视觉、像素和质量门禁后，实际新增 133 条 `knowledge_only` 资产，5 条已有完整证据、2 条视觉重复、24 条视觉疑似、5 条低质量均未入库。数据库资产增至 732，本批 embedding 133/133，第二次真实执行新增 0；本批 `allowed_for_bid/formal_bid_ready/parameter_fact_allowed` 均为 0，并全部排除 DOCX。原生 OOXML 抽取技术标 10 表、商务标 4 表，20 行规格候选保持 `DN/φ` 原单位且不覆盖现有检验报告参数。真实 stream 发现并修复 CPVC 复合参数误选承口值问题，最终正确回答 250.2～250.4 mm、15.2～15.3 mm和报告号 2024100312005501713。最终 Base+泰昌门禁、真实 stream、后端 413 项测试和 2 个子测试均 PASS。
0. **泰昌历史标书复用 P1-02 文件级证据包已完成。** 基于 P1-02 执行时的真实 PostgreSQL 与 P0-02 基线只读重建 16 个稳定业务证据包，共 149 页：CPVC/MPP 检验报告 2、主要试验设备及校准资料 6、管理体系证书 3、营业执照 1、审计报告 3、项目业绩“合同 + 中标通知书”1。16 包页序完整、缺页 0、重页 0；21 个重复 PDF/关键页 JPG 仅作 rendition。职业健康安全证书因 2026-06-18 到期阻断，设备/体系证书有效期与审计年度保留条件门禁，资格预审结果因无独立原件记为缺口。P1-01 后续新增的 133 条历史 Word 资产仍是 `knowledge_only + exclude_from_docx=true`，不追补为正式证据包页源；辽宁招标资料和河北豪乾参考稿也均未进入。
0. **泰昌历史标书复用 P0-07 通用流程兼容性边界已完成。** 新增项目级 `general/taichang_reuse` 模式契约，旧项目和现有上传入口默认保持 `general`；AI 解读、章节生成、历史解析重试和 DOCX 导出任务均从服务器项目记录固化模式，客户端 metadata 不能切换模式。`TAICHANG_REUSE_ENABLED=false` 仅关闭新建泰昌专版，不影响通用流程；数据库迁移未就绪时通用模式兼容旧表，泰昌模式禁止静默降级。本地 PostgreSQL 迁移后 48 个既有项目均保持 `general`；定向 43 项、后端全量 389 项及 2 个子测试、前端生产构建均通过。本轮未改变 RAG/正式资产/DOCX 内容规则，不触发召回或正式导出门禁；P0-06 已调整为低风险自动接收、仅异常项人工复核。
0. **泰昌历史标书复用 P0-06 已调整为分级自动接收与异常复核。** 泰昌主动提供两份历史标书用于系统整理和复用，视为来源处理授权，不再要求逐页人工批准。896 条候选分流为：169 条低风险知识资料按策略自动接收，290 条按 60 个证据组进入异常复核，437 条自动关联、阻断或仅参考；其中 270 条精确重复关联已有资产、2 条同证据不同载体关联同一证据包。自动接收项为绿色低碳 112、企业能力 35、生产制造能力 22，均保持 `knowledge_only + allowed_for_bid=false + formal_bid_ready=false`。当前策略批准和可进入提取/入库校验流程均为 169，人工批准为 0；尚未写数据库、未修改 RAG/metadata 或 DOCX 选图。P1-01 完成真实入库后必须执行标准增量门禁、真实 stream，涉及正式配图时执行 DOCX 审计。
0. **泰昌历史标书复用 P0-05 标签、分类与质量分级已完成。** 896 条候选已全部补齐主体、来源域、目标库、证据类型、产品族、分册/章节、敏感级别、质量等级、审核状态、去重状态和中文展示字段；P0-05 初始分类为 838 条 `review_only`、58 条 `restricted`，并生成 461 条原始复核队列。P0-06 v2 随后基于客户来源授权从中自动接收 169 条低风险资料，其余异常项继续复核。P0-05 的 27 项回归、重复执行和冻结输入哈希验证保持有效。
0. **泰昌历史标书复用 P0-04 四层去重已完成。** 已对两份历史标书的 896 条候选与 7,716 条冻结基线执行文件、视觉、文本和业务主键四层只读比对；从真实 `local_path` 重算 543 个视觉文件，避免把 staging JSON 指纹当图片哈希。结果为精确重复 270、视觉疑似 142、文本疑似 5、同证据不同载体 2、事实冲突 15、新候选 462。全部候选保持 `review_only + allowed_for_bid=false + promotion_eligible=false`，未写数据库、未改 metadata、未自动合并。18 项定向/前序回归通过，第二次执行结果一致，冻结输入哈希未变化。本轮不改变 RAG 语料、metadata、召回或 DOCX 选图，因此不触发 Base+泰昌全量召回门禁；P0-05 若发生审核入库或 metadata 变更必须执行标准增量门禁。
0. **泰昌历史标书复用 P0-03 两份 Word 只读解析已完成。** 已使用原生 DOCX XML/样式/表格/媒体关系和目录页码解析《技术补充文件》《商务补充文件》，未调用 MinerU/OCR、未写数据库。技术标盘点 368 页、73 章、10 表、352 个媒体文件并生成 486 条候选；商务标盘点 325 页、61 章、4 表、339 个媒体文件并生成 410 条候选。134 个章节全部具有来源属性，其中 39 个与 SL2655 样本实际位于第六章的格式内容直接匹配，其余保持评分展开、泰昌习惯补充、版式参考或待复核；该章节号仅是样本事实，后续项目须全文语义定位实际结构来源并生成项目动态骨架。所有候选均为 `review_only + allowed_for_bid=false`。
0. **泰昌历史标书复用 P0-02 现有数字资产基线已冻结。** 已从当前真实 PostgreSQL、泰昌 staging 和 `rag_seed` 原始资料生成 7,716 条统一快照：知识文档 77、分块 6,347、图片/附件资产 599、产品参数 36、项目业绩证据 2、原始文件 116、staging 候选 539。正式数据库资产/文档/分块审计问题 0，staging 候选 539 条全部保持隔离；识别原始资料重复拷贝 25 份、同内容知识文档冗余 6 条和测试资产残留 2 个，转入 P0-04/P0-05处理。本轮只读，未入库、未改 metadata 或召回策略。
...
0. **P0/P1 已完成：泰昌正式资料资产中文化与标书配图质量治理，阿里云测试环境复验和 RAG 门禁均通过。** 本地真实库已完成多轮审计和修复，最终真实图片资产 599、知识文档 77、文档分块 6347 的正式可见/RAG 可见问题均为 0；`run_20260627_local_formal_asset_regression` 完成真实服务回归：6 类 `/api/knowledge/search/stream` 禁用字段命中 0，Chrome 企业知识库页面和助手问答禁用字段命中 0，技术标/商务标真实 DOCX 导出 `forbidden_hits=[]`、`caption_page_hits=[]`、字段刷新 `refreshed`。阿里云执行修复后审计真实图片资产 596、知识文档 77、文档分块 6347 的正式可见/RAG 可见问题均为 0；`run_20260627_aliyun_browser_full_regression` 完成 Chrome 真实回归，企业知识库、产品库、资信库、知识库助手、6 类 stream 问答、技术标/商务标 DOCX 导出和下载文件 XML 审计均 PASS。已修复河北豪乾参考稿 metadata，并更新泰昌专项评测集；`run_20260627_taichang_formal_asset_cleanup_gate_v2` 标准门禁 PASS，Base qwen3 Recall@5 96.7%、Top1 100%、串扰 0，泰昌专项 qwen3 Recall@5 100%、Top1 100%、禁用关键词 0、串扰 0。
...
1. **产品库/资信库上传入口正式资产规则回归已通过。** `run_20260627_upload_entry_formal_asset_regression` 完成真实登录、真实 `/api/knowledge/assets/upload`、资产列表/详情、真实 `/api/knowledge/search/stream` 和测试资产清理；新上传测试资产返回 `泰昌MPP生产线资料`、`生产制造能力`、`泰昌/生产制造能力`，用户侧禁用字段命中 0，RAG 可召回。新增上传 payload 回归测试，`tests/test_knowledge_asset_upload_payload.py tests/test_rag_display_names.py tests/test_rag_retrieval.py` 为 40 passed；`run_20260627_upload_entry_formal_asset_gate` 标准增量门禁 PASS，Base qwen3 Recall@5 96.7%、泰昌专项 qwen3 Recall@5 100%、禁用关键词 0、跨 doc_role 串扰 0。
