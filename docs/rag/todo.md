...
19. **P1C-15 进行中：阿里云企业库展示与来源收敛收口。** 已完成一次性专项收敛：查询侧按证据类型桶收敛资质证书、绿色低碳、人员社保、检验报告、生产制造能力等来源；展示层不再把具体分类覆盖为“泰昌企业资料”；本地执行绿色低碳 metadata 修复后，221 条绿色/低碳资产和 1205 条 chunk 均归入 `green_low_carbon`。6 问真实 API 审计和标准门禁 `run_20260626_taichang_enterprise_source_scope_final` 均 PASS。阿里云线上 `ac01c14` 已通过真实 API + Chrome 页面核心回归，资质证书问答未再混入绿色低碳资料；本轮发现并修复前端参考来源标题优先级残留，剩余动作是发布新前端 commit 后复测页面“绿色低碳资料”参考来源标题，再关闭 P0。
