...
0. **泰昌历史标书复用 P1-04 章节—事实—证据映射已完成。** 已建立 19 个动态语义章节映射（技术标 12、商务标 7），状态分为已有资料 5、部分可用 10、缺原件 3、人工确认 1；只读复用 15 个唯一证据包、36 行原始报告参数、17 行安全台账投影和 P1-01 知识资产，不复制业务资产。CPVC/MPP 可定位结构化参数和完整检验报告；N-HAP/UPVC 缺原件、过期职业健康安全证书、资格预审缺口、2024 审计编号待复核、授权签章人工确认等门禁均已固化。人员受限明细纳入 0，固定历史章节号纳入 0。专项 29 passed、后端全量 432 passed 及 2 个子测试通过；本轮不改数据库、metadata、召回、接口或 DOCX，因此不触发真实链路门禁。下一步进入 P1-05 RAG 入库与回归门禁。
0. **泰昌历史标书复用 P1-03 结构化参数和业务台账已完成。** 已生成 84 行统一台账，并只读关联 36 行 CPVC/MPP 原始报告参数、20 行历史技术候选、2 行项目业绩证据和 16 个文件级证据包；产品参数已补来源页码。N-HAP/UPVC 缺原始报告、2024 年审计报告编号待复核、知识产权无可信原件均保持显式缺口；职业健康安全证书已过期阻断；67 条人员明细保持 `restricted/internal_only` 且仅写本地 staging，版本库和普通问答只保存 17 行安全投影与脱敏汇总。真实 stream 首轮发现并修复“多年度台账来源合并”和“N-HAP/UPVC 串入 CPVC”问题，最终 7/7 PASS；Base+泰昌标准门禁 PASS；专项 18 passed、后端全量 423 passed 及 2 个子测试通过。本轮数据库写入 0、资产提升 0、DOCX 选图变更 0。下一步进入 P1-04 章节—事实—证据映射。
