...
19. **P1C-15 已完成：阿里云企业库展示与来源收敛收口。** 查询侧已按证据类型桶收敛资质证书、绿色低碳、人员社保、检验报告、生产制造能力等来源；展示层不再把具体分类覆盖为“泰昌企业资料”；本地执行绿色低碳 metadata 修复后，221 条绿色/低碳资产和 1205 条 chunk 均归入 `green_low_carbon`。6 问真实 API 审计和标准门禁 `run_20260626_taichang_enterprise_source_scope_final` 均 PASS；阿里云线上 `ac01c14` 后端/API 核心回归 PASS，`cfd1e4e03dd5` 前端标题补丁发布后 Chrome 页面复验 PASS。资质证书问答未再混入绿色低碳来源，绿色低碳参考来源标题已显示“绿色发展规划报告”“碳足迹报告”。
