...
0. **泰昌历史标书复用 P0-07 通用流程兼容性边界已完成。** 新增项目级 `general/taichang_reuse` 模式契约，旧项目和现有上传入口默认保持 `general`；AI 解读、章节生成、历史解析重试和 DOCX 导出任务均从服务器项目记录固化模式，客户端 metadata 不能切换模式。`TAICHANG_REUSE_ENABLED=false` 仅关闭新建泰昌专版，不影响通用流程；数据库迁移未就绪时通用模式兼容旧表，泰昌模式禁止静默降级。本地 PostgreSQL 迁移后 48 个既有项目均保持 `general`；定向 43 项、后端全量 389 项及 2 个子测试、前端生产构建均通过。本轮未改变 RAG/正式资产/DOCX 内容规则，不触发召回或正式导出门禁；P0-06 继续等待泰昌资料管理人员审批。
0. **泰昌历史标书复用 P0-06 技术准备已完成，业务审批待泰昌确认。** 896 条候选全部分流：461 条按 65 个完整报告/证据包进入中文 Excel 人工复核，435 条自动处理为禁止新增 270、禁止复用 15、保留历史参考 148、禁止独立入库 2。审批表默认无批准；32 条敏感、1 条受限资料不能通过普通审批直接入库，3 条已过期职业健康安全管理体系证书强制延后补证，参数表获批后仍必须结构化抽取。审批完整性和各类安全门禁已通过 37 项回归及 LibreOffice 转存回读。当前已批准和可入库均为 0，未写数据库、未修改 RAG/metadata 或 DOCX 选图；待泰昌资料管理人员填写审批表后，实际入库必须执行标准增量门禁、真实 stream 和必要的 DOCX 审计。
0. **泰昌历史标书复用 P0-05 标签、分类与质量分级已完成。** 896 条候选已全部补齐主体、来源域、目标库、证据类型、产品族、分册/章节、敏感级别、质量等级、审核状态、去重状态和中文展示字段；838 条保持 `review_only`、58 条为 `restricted`。已生成 461 条 P0-06 人工复核队列，270 条精确重复和 15 条事实冲突被阻断，正式增量入库候选为 0。27 项回归通过，重复执行结果一致，冻结输入哈希未变化。本轮未写数据库、未修改 RAG/metadata 或 DOCX 选图，因此不触发全量召回和正式导出门禁；P0-06 实际批准入库后必须执行标准增量门禁、真实 stream 和必要的 DOCX 审计。
