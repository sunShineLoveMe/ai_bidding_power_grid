...
0. **下一任务调整为 P2-01 项目级招标编制规则、动态骨架与历史参考骨架差异对照。** P0/P1 数据基线已经完成，但“入库/可召回”不再作为终局完成标准。P2 必须按“动态骨架 → 正文内容复用 → 固定表单 → 正式证据包 → 导出门禁 → DOCX/PDF 验收”推进；P2-02 要将 36 行参数、84 行业务台账、16 个证据包、2 行业绩证据、166 个结构化分块和 133 条历史知识资产定向映射到技术标/商务标章节，并输出章节内容使用清单。133 条 `knowledge_only` 资产可作为正文写作线索，但继续禁止直接作为正式图片或附件插入 DOCX。
