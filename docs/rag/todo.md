...
0. **P0 已完成：泰昌正式资料资产中文化与标书配图质量治理，阿里云测试环境复验通过。** 本地真实库已完成多轮审计和修复，最终真实图片资产 599、知识文档 77、文档分块 6347 的正式可见/RAG 可见问题均为 0；`run_20260627_local_formal_asset_regression` 完成真实服务回归：6 类 `/api/knowledge/search/stream` 禁用字段命中 0，Chrome 企业知识库页面和助手问答禁用字段命中 0，技术标/商务标真实 DOCX 导出 `forbidden_hits=[]`、`caption_page_hits=[]`、字段刷新 `refreshed`。阿里云执行修复后审计真实图片资产 596、知识文档 77、文档分块 6347 的正式可见/RAG 可见问题均为 0；`run_20260627_aliyun_browser_full_regression` 完成 Chrome 真实回归，企业知识库、产品库、资信库、知识库助手、6 类 stream 问答、技术标/商务标 DOCX 导出和下载文件 XML 审计均 PASS。Base 增量门禁未退化；泰昌专项门禁仍需更新旧评测集，原因是旧用例期待已被正式隔离的历史资产索引/解析中间 chunk 被召回。
