...
| P1C-15 | [~] | 阿里云企业库展示与来源收敛收口 | `backend/api/knowledge.py`、`backend/rag/retrieval.py`、`docker-compose.yml`、`docs/rag/runs/run_20260626_aliyun_enterprise_source_convergence_final_review.md`、`docs/rag/runs/run_20260624_aliyun_online_rag_regression.md` | 本地代码与真实链路已收口：资质证书来源不再混入 ESG/绿色发展/废水类资料，CPVC 检验报告按内径250和报告编号限定覆盖范围；Base+泰昌专项门禁 PASS，资质证书和 CPVC stream PASS。当前云上 `:8080` 健康，公网 80 仍 `Empty reply from server`，需发布后复测再关闭 |
...
19. **P1C-15 进行中：阿里云企业库展示与来源收敛收口。** 本地已修复资质证书来源混入 ESG/废水报告、CPVC 报告规格覆盖措辞冲突，并完成 `FRONTEND_HTTP_PORT=80` 入口配置文档；`run_20260626_aliyun_enterprise_source_convergence_final` 增量门禁和真实 stream 通过。剩余外部动作是发布到阿里云 ECS 并复测 `:8080` 与 80 入口，确认后再关闭 P0。
