...
0. **P0 本地已确认，待阿里云复验：泰昌正式资料资产中文化与标书配图质量治理。** 本地真实库已完成多轮审计和修复，最终真实图片资产 599、知识文档 77、文档分块 6347 的正式可见/RAG 可见问题均为 0；`run_20260627_local_formal_asset_regression` 完成真实服务回归：6 类 `/api/knowledge/search/stream` 禁用字段命中 0，Chrome 企业知识库页面和助手问答禁用字段命中 0，技术标/商务标真实 DOCX 导出 `forbidden_hits=[]`、`caption_page_hits=[]`、字段刷新 `refreshed`。Base 增量门禁未退化，但泰昌专项门禁失败，原因是旧评测集仍期待历史资产索引/解析中间 chunk 被召回；下一步需更新专项评测集，并在阿里云线上执行同等修复和真实浏览器复验。
