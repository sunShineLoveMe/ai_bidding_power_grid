...
0. **P2-01 已完成，当前进入 P2-02 泰昌专版分册骨架与正文内容复用。** SL2655 样本已形成 43 条项目规则和 19 条成稿目录项，价格按包、商务/技术按分标隔离；24 条不适用、继承、禁止或未勾选内容未进入目录。系统已接入当次招标规则优先的动态骨架、来源追溯、确定性历史差异和用户可见核定提示；MPP `250×22 / 176%` 不覆盖项目 `200×14 / ≥200%`，正式导出保持阻断。后端全量 446 passed、前端构建、真实目录 SSE、浏览器回归、真实知识库流和 Base 30 + 泰昌专项 30 双模式门禁均 PASS。P2-02 继续将 36 行参数、84 行业务台账、16 个证据包、2 行业绩证据、166 个结构化分块和 133 条历史知识资产定向映射到当前动态章节，并生成 `chapter_content_manifest`；`knowledge_only` 仍不得直接作为正式图片或附件插入 DOCX。
