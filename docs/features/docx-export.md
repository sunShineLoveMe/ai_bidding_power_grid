...
2026-06-06 起，泰昌 MVP 导出默认模板为 `sgcc_taichang_bid`：

- 默认投标人：`河北泰昌电力器材科技有限公司`。
- 导出标题、文件名、封面、页眉均使用泰昌投标人口径，不再沿用“招标文件”口径。
- 未显式传 `withImages` 时，DOCX 下载默认图文并茂。
- 自动插图只使用泰昌企业事实资产，排除辽宁招标要求和河北豪乾参考稿。
- 自动插图上限为 24 张，同一资产整份文档只插入一次。
- 页面参数默认 A4、上下 `2.0cm`、左右 `3.18cm`、正文宋体 `10.5pt`、固定行距 `20pt`。
- 目录页标题为 `目  录`，目录正文宋体 `10.5pt`，页码使用 `PAGEREF` 字段刷新。
