...
2026-06-25 起，泰昌 MVP 导出默认模板调整为 `formal_bid_standard`。该模板按用户确认的国家电网投标文件通用正式排版口径执行：

- 默认投标人：`河北泰昌电力器材科技有限公司`。
- 导出标题、文件名、封面均使用泰昌投标人口径，不再沿用“招标文件”口径。
- 未显式传 `withImages` 时，DOCX 下载默认图文并茂。
- 自动插图只使用泰昌企业事实资产，排除辽宁招标要求和河北豪乾参考稿。
- 自动插图上限为 24 张，同一资产整份文档只插入一次。
- 页面参数默认 A4、上/下 `2.5cm`、左 `2.8cm`、右 `2.5cm`。
- 正文默认仿宋_GB2312 `14pt`、固定行距 `22pt`、首行缩进 `2` 字符。
- 一级标题黑体 `22pt` 居中，二级标题黑体 `15pt` 左对齐，三级标题楷体_GB2312 `14pt` 加粗。
- 表格文字仿宋_GB2312 `12pt`，表头加粗，表格全宽、固定布局、表头跨页重复。
- 目录页标题为 `目  录`，页码使用 `PAGEREF` 字段刷新。
- 页眉左侧为项目名称、右侧为文件类型；页脚为 `第 X 页 共 Y 页` 字段。
