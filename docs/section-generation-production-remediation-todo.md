...
最近更新：2026-06-26，完成 Chrome 真实浏览器从上传辽宁真实招标文件到 75/75 个叶子正文生成的全流程回归；正文任务最终 completed，但首轮出现 1 个 `partial_generated`，通过页面“批量续写草稿”补完，后续按总账 P1 跟踪自动续写和入口体验收口。
...
## 2026-06-26 真实浏览器全流程正文生成回归

- 回归记录：`docs/development/runs/run_20260626_real_browser_full_bid_body_flow.md`
- 测试项目：`4d632dbe-f6e6-4066-8fe2-929ecb54ba1d`
- 正文任务：`86730fcc-4f4b-40e5-a5e5-2335889ee693`
- 结果：上传真实辽宁 DOCX -> 解析 -> 招标解读 -> 分册大纲 -> 投标确认 -> 一键编写全文 -> partial 草稿续写 -> `75/75` 叶子正文完成。
- 数据库核验：`bid_generation_tasks.status=completed`，`bid_generation_task_items done=75`，叶子章节 `leaf_with_content=75/75`，默认占位/失败标记命中 0。
- 暴露问题：项目状态仍为 `uploaded` 导致首页/历史记录显示“解析中”；投标确认页首次进入需要手动刷新报告；首轮出现 1 个模型 idle timeout partial，需要手动批量续写。
- 优先级同步：上述流程体验和状态同步问题已进入 `docs/development/master-todo.md` P1“全流程项目状态同步与投标确认入口收口”。
