...
VOLUME_DEFINITIONS: dict[str, dict[str, str]] = {
    ...
    "technical": {
        "name": "技术标",
        "description": "技术响应、产品参数、生产检验、质量控制、供货交付和售后服务",
    },
    "business": {
        "name": "商务标",
...
VOLUME_GENERATION_STRATEGIES: dict[str, dict[str, Any]] = {
    "technical": {
        "focus": [
            "围绕产品技术参数、生产制造、试验检测、质量控制、供货交付和售后服务展开。",
            "优先回应技术规范、标准参数、评分办法、供货范围和质量风险。",
            "可以使用产品库、检验报告、生产线、检测设备、质量体系和工艺流程作为支撑。",
        ],
        "constraints": [
            "不得把商务承诺、投标函格式内容写成技术响应主体。",
            "不得生成施工组织、建造师、安全生产许可证、BIM、水利施工、安装总承包等工程承包内容。",
            "涉及产品参数、检验指标和供货资源时，优先使用已确认变量和企业事实；缺失决策字段不得编造。",
            "流程图、产品图、设备图只作为辅助说明，不能替代文字响应。",
        ],
        "retrieval_hint": "优先召回技术规范、货物清单、产品库、检验报告、设备图片、工艺流程和质量管理资料。",
        "image_policy": "可以自动插入产品图、生产线图、检测设备图、检验报告和工艺图；图片应与章节技术内容直接相关。",
    },
    "business": {
        "focus": [
        ...
        "constraints": [
            ...
            "金额、日期、签章、保证金等信息必须优先使用已确认字段；未确认时仅在系统检查中提示，不得在正式正文铺开占位符。",
            "不要插入与商务条款无关的产品图或施工图。",
        ],
    ...
    "qualification": {
        ...
        "constraints": [
            ...
            "缺失证照、人员、业绩信息应在系统缺件清单中提示，正式正文使用简短说明，不得铺开占位符。",
            "插入图片时使用正式资料题注，不展示样张、脱敏、内部路径或需人工替换等系统语言。",
        ],
    ...
    "price": {
        ...
        "constraints": [
            ...
            "涉及金额或清单数据时优先使用已确认字段；未确认时生成正式报价口径说明，不在正文写造价人员复核提示。",
    ...
    "attachment": {
        ...
        "constraints": [
            "不得把附件清单写成正式证照或业绩事实。",
            "缺失附件进入系统缺件清单，不得在正式正文中使用【待补充】。",
            "图片和附件题注只保留正式资料名称和用途，不展示来源路径、内部用途或脱敏说明。",
        ],
